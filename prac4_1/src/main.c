...
int main(){
	...
	//Iniciar el reloj monotonico
	InitMonotonicClock(date_time_to_Y2K(16, 3, 19, 0, 0, 1));

	/**Iniciar el servicio de temporizacion para que se generen 10 interrupciones por segundo,
	 * y en cada interrupcion se invoque a la rutina que actualiza el reloj monotonico.
	 */
	InitTimingService(TICKS_PER_SECOND, IRQHandlerUpdateMonotonicClock);
